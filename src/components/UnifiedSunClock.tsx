...
const UnifiedSunClock: React.FC<UnifiedSunClockProps> = ({
  ...
  // Sun-times math now lives in src/lib/sunMath.ts so the clock and the
  // streak / session logic share a single source of truth.
  ...
  // FIXED: Proper tracking button handler using the hook
  const handleToggleTracking = async () => {
    console.log(`🔄 Button clicked! Current state: ${isTracking}`);
    ...
    if (isTracking) {
      ...
      let sessionType: "morning" | "evening" | "manual" = "manual";
      if (inMorningPrime) sessionType = "morning";
      if (inEveningPrime) sessionType = "evening";

      const sessionId = startSession(sessionType);
  ...
  // Get current session elapsed time from hook
  ...
  return (
    ...
    >
      <div className="relative z-10 max-w-2xl mx-auto text-center">
          ...
          <div className="relative w-80 h-80">
            ...
            {clockFormat === "main" &&
              Array.from({ length: 24 }, (_, i) => {
                ...
                const x1 =
                  160 + Math.cos((angle * Math.PI) / 180) * innerRadius;
                const y1 =
                  160 + Math.sin((angle * Math.PI) / 180) * innerRadius;
                const x2 =
                  160 + Math.cos((angle * Math.PI) / 180) * outerRadius;
                const y2 =
                  160 + Math.sin((angle * Math.PI) / 180) * outerRadius;
        ...
        {/* FIXED: Working Start/Stop Light Tracking Button */}
        <div className="mb-2">
          <button
            onClick={handleToggleTracking}
            className="px-4 py-2 bg-white/10 backdrop-blur-md text-white/80 text-sm rounded-full hover:bg-white/20 transition-all duration-300 border border-white/20"
            style={{
